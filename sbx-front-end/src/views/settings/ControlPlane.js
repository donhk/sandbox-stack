...
const ControlPlane = () => {
  ...
  const reloadVms = async () => {
    ...
    setShowCat(true);
    try {
      await axios.post(config.baseUrl + '/api/machines/scan');
    } catch (error) {
      console.error('Error scanning VMs:', error);
    } finally {
      setLoading(false);
      setShowCat(false);
    }
  ...
  const destroyVm = async () => {
    ...
    try {
      const time = Date.now();
      const resp = (
        await axios.delete(config.baseUrl + `/api/machine/${vmNameToDestroy}`)
      ).data;
      setDestroyElapsedMs(
        `✅ VM ${resp.uuid} successfully destroyed in ${Date.now() - time} ms`,
      );
      setVmNameToDestroy('');
      // Show alert
  ...
  return (
    <>
      ...
      >
        ...
        <COffcanvasBody>{destroyElapsedMs}</COffcanvasBody>
      </COffcanvas>
